<template>
  <div :class="`tp-product-item-2 `">
    <div class="tp-product-thumb-2 p-relative z-index-1 fix w-img rounded-md"
         style="background-color: #f2f3f5">
      <nuxt-link :href="`/product-details/${item?.name}/${item?.id}`">
        <img class="object-cover h-64"
      ...
      <div class="tp-product-badge">
        <span v-if="!item?.in_stock"
              class="product-hot">out-of-stock</span>
      </div>
      <div class="tp-product-badge">
        <span v-if="item?.on_sale"
              class="product-trending">Offer</span>
      </div>

      <!-- product action -->
      <div class="w-full border-t flex justify-between px-2 pt-2 pb-1">
        <button v-if="!isItemInCart(item)"
                @click="cartStore.addCartProduct(item)"
                type="button"
                :class="`tp-product-action-btn-2 flex justify-center items-center tp-product-add-cart-btn ${isItemInCart(item) ? 'active' : ''
                  }`">
          <svg-add-cart />
          <span class="tp-product-tooltip tp-product-tooltip-right">Add to Cart</span>
        </button>
        <nuxt-link v-if="isItemInCart(item)"
                   href="/cart"
                   :class="` flex items-center justify-center tp-product-action-btn-2 tp-product-add-cart-btn ${isItemInCart(item) ? 'active' : ''
                    }`">
          <svg-add-cart />
        ...
        <button @click="wishlistStore.add_wishlist_product(item)"
                type="button"
                :class="` flex items-center justify-center tp-product-action-btn-2 tp-product-add-to-wishlist-btn ${isItemInWishlist(item) ? 'active' : ''
                  }`">
          <svg-wishlist />
    ...
    <div class="tp-product-content-2 pt-15">
      ...
      <h3 class="tp-product-title-2">
        <nuxt-link :href="`/product-details/${item?.name}/${item?.id}`">{{ item?.name }} -
          <span class="text-gray-400">{{ item?.inventory }} Left</span>
        </nuxt-link>
      </h3>

      <p :title="normalizedDescription"
         class="text-truncate text-wrap w-full">
        <span class="">{{ truncatedDescription }}</span>
      </p>
      <div class="tp-product-rating-icon tp-product-rating-icon-2">
        <span v-for="item in parseInt(item?.review_stats?.average_rating)"
              :key="item"><i class="fa-solid fa-star"></i></span>
        <span v-for="item in 5 - parseInt(item?.review_stats?.average_rating)"><i class="fa-regular fa-star"></i></span>

        <span class="ml-2 text-black">{{ item?.review_stats.average_rating }} / 5 Stars -
...
// Add these computed properties to normalize text and prevent hydration mismatch
const normalizedDescription = computed(() => {
  if (!props.item?.description) return '';
  // Replace \r\n with space to prevent hydration mismatch
