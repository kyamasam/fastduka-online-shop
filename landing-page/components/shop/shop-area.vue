...
const generateFilterString = () => {
  ...
  Object.keys(store.filterObj).map((key, index) => {
    ...
    if (key === "category") {
      let categories_list = "";
      let category_len = store.filterObj.category.length;
      if (category_len) {
        store.filterObj.category.map((i: any, index: number) => {
          categories_list += `${i}`;
          if (index + 1 !== category_len) {
            categories_list += ",";
          }
        });
    ...
    if (key === "page") {
      tempFilterString += `${filter_prefix}${key}= ${store.filterObj?.page}`;
    }
  });
...
const fetchProducts = async () => {
  ...
  pending.value = true;
  let product_type = route?.name === "shop" ? "MEAT" : "LIQUOR";
  try {
