<template>
    <div>
        ...
        <div v-else-if="data">
            <!-- Breadcrumb -->
            <BlogBreadcrumb v-if="currentBlog" :blog="currentBlog" />

            <!-- Blog Details Area -->
            <BlogDetailsArea v-if="currentBlog" :blog="currentBlog" />
                ...
                <section class="py-12">
                    <div class="container mx-auto px-4">
                        ...
                        <div class="flex flex-col lg:flex-row gap-6 mb-8 items-start lg:items-end justify-between">
                            <div class="flex flex-col sm:flex-row gap-4 w-full lg:w-auto">
                                <div class="flex flex-col gap-2">
                                    ...
                                    <select id="category-filter" v-model="selectedCategory" @change="filterBlogs"
                                        class="px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 h-12">
                                ...
                                <div class="flex flex-col gap-2 relative">
                                    ...
                                    <div class="relative">
                                        <input type="text" v-model="searchQuery" placeholder="Search blogs..."
                                            class="pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 w-full h-12"
                                            @input="onSearchInput" />
                                        <div
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-12">
                            <article v-for="blog in data.results" :key="blog.id"
                                class="bg-white rounded-xl shadow-sm hover:shadow-lg transition-all duration-300 cursor-pointer overflow-hidden group"
                                @click="$router.push(`/blogs/${blog.slug}/${blog.id}`)">
                                <div class="relative overflow-hidden">
                                    <img :src="blog.cover_photo || '/images/blog-placeholder.jpg'" :alt="blog.title"
                                        loading="lazy"
                                ...
                                <div class="p-6">
                                    <h3
                                        class="text-xl font-semibold text-gray-900 mb-3 line-clamp-2 group-hover:text-blue-600 transition-colors">
                                        {{ blog.title }}
                                    </h3>
                                    ...
                                    <button
                                        class="w-full py-2 bg-gray-100 text-gray-700 rounded-lg font-medium hover:bg-blue-600 hover:text-white transition-colors">
                                        Read More
                                    </button>
                                </div>
                            </article>
                        ...
                        <div v-if="data.count > pagination.pageSize" class="flex items-center justify-center gap-4">
                            <button :disabled="!data.previous" @click="loadPage(data.previous)"
                            ...
                            <button :disabled="!data.next" @click="loadPage(data.next)"
                                class="px-4 py-2 border border-gray-300 rounded-lg disabled:opacity-50 disabled:cursor-not-allowed hover:bg-gray-50 transition-colors">
                                Next
                            </button>
                        </div>
