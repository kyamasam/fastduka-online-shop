import BlogCategoryList from "@/view/blogs/BlogCategoryList.vue";
import BlogList from "@/view/blogs/BlogList.vue";
import EditBlogView from "@/view/blogs/EditBlogView.vue";
import CategoryList from "@/view/categories/CategoryList.vue";
import CategoryManagement from "@/view/categories/CategoryManagement.vue";
import CreateCategoryView from "@/view/categories/CreateCategoryView.vue";
import EditCategoryView from "@/view/categories/EditCategoryView.vue";
import AddUpdateInventory from "@/view/inventory/AddUpdateInventory.vue";
import InventoryBatches from "@/view/inventory/InventoryBatches.vue";
import InventoryList from "@/view/inventory/InventoryList.vue";
import InventoryManagement from "@/view/inventory/InventoryManagement.vue";
import UploadProductImages from "@/view/inventory/UploadProductImages.vue";
import OnboardingFlow from "@/view/onboarding/OnboardingFlow.vue";
import OrderList from "@/view/orders/OrderList.vue";
import OrderManagement from "@/view/orders/OrderManagement.vue";
import CreateProductView from "@/view/products/CreateProductView.vue";
import ProductList from "@/view/products/ProductList.vue";
import ProductManagement from "@/view/products/ProductManagement.vue";
import ProductForm from "@/view/products/components/ProductForm.vue";
import UserProfile from "@/view/profile/UserProfile.vue";
import MiniRiderList from "@/view/riders/MiniRiderList.vue";
import RegisterRiderView from "@/view/riders/RegisterRiderView.vue";
import SelectRirder from "@/view/riders/SelectRirder.vue";
import RiderForm from "@/view/riders/components/RiderForm.vue";
import SiteSettings from "@/view/settings/index.vue";
import CreateUser from "@/view/users/CreateUser.vue";
import UserList from "@/view/users/UserList.vue";
import CreateVendorView from "@/view/vendors/CreateVendorView.vue";
import VendorList from "@/view/vendors/VendorList.vue";
import VendorManagement from "@/view/vendors/VendorManagement.vue";
import VendorMembers from "@/view/vendors/VendorMembers.vue";
import MemberShipForm from "@/view/vendors/components/MemberShipForm.vue";
import VendorForm from "@/view/vendors/components/VendorForm.vue";
import CustomerPayments from "@/view/wallet/CustomerPayments.vue";
import WalletIndex from "@/view/wallet/WalletIndex";
import payments from "@/view/wallet/payments";
import withdrawals from "@/view/wallet/withdrawals";
import { createRouter, createWebHistory } from "vue-router";
import Dashboard from "../layout/Dashboard.vue";
import BlankPage from "../view/auth/BlankPage.vue";
import PosPage from "@/view/pos/PosPage.vue";
const routes = [
  {
    path: "/:pathMatch(.*)",
    name: "not-found",
    component: () => import("../view/auth/404.vue"),
    meta: {
      requiresAuth: false,
      allowed_roles: [
        "client",
        "photographer",
        "admin",
        "owner",
        "super_admin",
      ],
    },
  },
  {
    path: "/",
    name: "login",
    component: () => import("../view/auth/Login.vue"),
    meta: {
      requiresAuth: false,
      allowed_roles: ["photographer", "admin", "owner", "client"],
    },
  },
  {
    path: "/register",
    name: "register",
    component: () => import("../view/auth/Register.vue"),
    meta: {
      requiresAuth: false,
      allowed_roles: ["photographer", "admin", "owner", "client"],
    },
  },
  {
    path: "/unauthorised",
    name: "unauthorised",
    component: () => import("../view/auth/Unauthorised.vue"),
    meta: {
      requiresAuth: false,
      allowed_roles: ["photographer", "admin", "owner", "client"],
    },
  },
  {
    path: "/forgot-password",
    name: "forgotPassword",
    component: () => import("../view/auth/ForgotPassword.vue"),
    meta: {
      requiresAuth: false,
    },
  },
  {
    path: "/renew-password/:passwordResetCode",
    name: "renewPassword",
    component: () => import("../view/auth/RenewPassword.vue"),
    meta: {
      requiresAuth: false,
      allowed_roles: ["photographer", "admin", "owner", "client"],
    },
  },
  {
    path: "/onboarding",
    name: "onboarding",
    component: OnboardingFlow,
    meta: {
      title: "Onboarding",
      showInSideBar: false,
    },
  },
  {
    path: "/dashboard",
    name: "dashboard",
    component: Dashboard,
    meta: {
      showInSideBar: false,
      verboseName: "Dashboard",
      level: 1,
      icon:
        '<svg width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">\n' +
        '<path d="M12.8864 0.25H0.613636C0.274219 0.25 0 0.465458 0 0.732143V13.2679C0 13.5345 0.274219 13.75 0.613636 13.75H12.8864C13.2258 13.75 13.5 13.5345 13.5 13.2679V0.732143C13.5 0.465458 13.2258 0.25 12.8864 0.25ZM1.38068 1.33482H12.1193V4.46875H1.38068V1.33482ZM12.1193 8.56696H1.38068V5.43304H12.1193V8.56696ZM12.1193 12.6652H1.38068V9.53125H12.1193V12.6652ZM6.44318 2.41964H2.91477C2.8304 2.41964 2.76136 2.47388 2.76136 2.54018V3.26339C2.76136 3.32969 2.8304 3.38393 2.91477 3.38393H6.44318C6.52756 3.38393 6.59659 3.32969 6.59659 3.26339V2.54018C6.59659 2.47388 6.52756 2.41964 6.44318 2.41964ZM2.91477 7.48214H6.44318C6.52756 7.48214 6.59659 7.4279 6.59659 7.36161V6.63839C6.59659 6.5721 6.52756 6.51786 6.44318 6.51786H2.91477C2.8304 6.51786 2.76136 6.5721 2.76136 6.63839V7.36161C2.76136 7.4279 2.8304 7.48214 2.91477 7.48214ZM9.20455 11.1585C9.20455 11.3183 9.28536 11.4716 9.42921 11.5846C9.57306 11.6977 9.76816 11.7612 9.97159 11.7612C10.175 11.7612 10.3701 11.6977 10.514 11.5846C10.6578 11.4716 10.7386 11.3183 10.7386 11.1585C10.7386 10.9986 10.6578 10.8453 10.514 10.7323C10.3701 10.6193 10.175 10.5558 9.97159 10.5558C9.76816 10.5558 9.57306 10.6193 9.42921 10.7323C9.28536 10.8453 9.20455 10.9986 9.20455 11.1585Z" fill="currentColor"/>\n' +
        "</svg>\n",
      requiresAuth: false,
      allowed_roles: [
        "photographer",
        "admin",
        "owner",
        "client",
        "business_owner",
      ],
    },
    children: [
      {
        path: "home",
        name: "home",
        component: BlankPage,
        meta: {
          showInSideBar: false,
          verboseName: "Home",
          requiresAuth: true,
          allowed_roles: ["photographer", "admin", "owner", "client"],
          level: 1,
          icon:
            '<svg width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">\n' +
            '<path d="M12.8864 0.25H0.613636C0.274219 0.25 0 0.465458 0 0.732143V13.2679C0 13.5345 0.274219 13.75 0.613636 13.75H12.8864C13.2258 13.75 13.5 13.5345 13.5 13.2679V0.732143C13.5 0.465458 13.2258 0.25 12.8864 0.25ZM1.38068 1.33482H12.1193V4.46875H1.38068V1.33482ZM12.1193 8.56696H1.38068V5.43304H12.1193V8.56696ZM12.1193 12.6652H1.38068V9.53125H12.1193V12.6652ZM6.44318 2.41964H2.91477C2.8304 2.41964 2.76136 2.47388 2.76136 2.54018V3.26339C2.76136 3.32969 2.8304 3.38393 2.91477 3.38393H6.44318C6.52756 3.38393 6.59659 3.32969 6.59659 3.26339V2.54018C6.59659 2.47388 6.52756 2.41964 6.44318 2.41964ZM2.91477 7.48214H6.44318C6.52756 7.48214 6.59659 7.4279 6.59659 7.36161V6.63839C6.59659 6.5721 6.52756 6.51786 6.44318 6.51786H2.91477C2.8304 6.51786 2.76136 6.5721 2.76136 6.63839V7.36161C2.76136 7.4279 2.8304 7.48214 2.91477 7.48214ZM9.20455 11.1585C9.20455 11.3183 9.28536 11.4716 9.42921 11.5846C9.57306 11.6977 9.76816 11.7612 9.97159 11.7612C10.175 11.7612 10.3701 11.6977 10.514 11.5846C10.6578 11.4716 10.7386 11.3183 10.7386 11.1585C10.7386 10.9986 10.6578 10.8453 10.514 10.7323C10.3701 10.6193 10.175 10.5558 9.97159 10.5558C9.76816 10.5558 9.57306 10.6193 9.42921 10.7323C9.28536 10.8453 9.20455 10.9986 9.20455 11.1585Z" fill="currentColor"/>\n' +
            "</svg>\n",
        },
      },
      {
        path: "inventory",
        name: "inventory",
        component: InventoryList,
        meta: {
          showInSideBar: true,
          verboseName: "Inventory",
          requiresAuth: true,
          allowed_roles: ["admin", "owner", "client"],
          level: 1,
          icon:
            '<svg width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">\n' +
            '<path d="M12.8864 0.25H0.613636C0.274219 0.25 0 0.465458 0 0.732143V13.2679C0 13.5345 0.274219 13.75 0.613636 13.75H12.8864C13.2258 13.75 13.5 13.5345 13.5 13.2679V0.732143C13.5 0.465458 13.2258 0.25 12.8864 0.25ZM1.38068 1.33482H12.1193V4.46875H1.38068V1.33482ZM12.1193 8.56696H1.38068V5.43304H12.1193V8.56696ZM12.1193 12.6652H1.38068V9.53125H12.1193V12.6652ZM6.44318 2.41964H2.91477C2.8304 2.41964 2.76136 2.47388 2.76136 2.54018V3.26339C2.76136 3.32969 2.8304 3.38393 2.91477 3.38393H6.44318C6.52756 3.38393 6.59659 3.32969 6.59659 3.26339V2.54018C6.59659 2.47388 6.52756 2.41964 6.44318 2.41964ZM2.91477 7.48214H6.44318C6.52756 7.48214 6.59659 7.4279 6.59659 7.36161V6.63839C6.59659 6.5721 6.52756 6.51786 6.44318 6.51786H2.91477C2.8304 6.51786 2.76136 6.5721 2.76136 6.63839V7.36161C2.76136 7.4279 2.8304 7.48214 2.91477 7.48214ZM9.20455 11.1585C9.20455 11.3183 9.28536 11.4716 9.42921 11.5846C9.57306 11.6977 9.76816 11.7612 9.97159 11.7612C10.175 11.7612 10.3701 11.6977 10.514 11.5846C10.6578 11.4716 10.7386 11.3183 10.7386 11.1585C10.7386 10.9986 10.6578 10.8453 10.514 10.7323C10.3701 10.6193 10.175 10.5558 9.97159 10.5558C9.76816 10.5558 9.57306 10.6193 9.42921 10.7323C9.28536 10.8453 9.20455 10.9986 9.20455 11.1585Z" fill="currentColor"/>\n' +
            "</svg>\n",
        },
        children: [
          {
            path: "management/:productId",
            name: "inventory-management",
            component: InventoryManagement,
            meta: {
              showInSideBar: false,
              verboseName: "Inventory Details",
              requiresAuth: true,
              allowed_roles: ["admin", "owner", "client"],
              level: 1,
            },
            children: [
              {
                path: "manage-inventory",
                name: "manage-inventory",
                component: AddUpdateInventory,
                meta: {
                  showInSideBar: false,
                  verboseName: "Manage Inventory",
                  requiresAuth: true,
                  allowed_roles: ["admin", "owner", "client"],
                  level: 1,
                },
              },
              {
                path: "inventory-batches",
                name: "inventory-batches",
                component: InventoryBatches,
                meta: {
                  showInSideBar: false,
                  verboseName: "Inventory Batches",
                  requiresAuth: true,
                  allowed_roles: ["admin", "owner", "client"],
                  level: 1,
                },
              },
            ],
          },
        ],
      },
      {
        path: "products",
        name: "products",
        component: ProductList,
        meta: {
          showInSideBar: false,
          verboseName: "Home",
          requiresAuth: true,
          allowed_roles: ["photographer", "admin", "owner", "client"],
          level: 1,
          icon:
            '<svg width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">\n' +
            '<path d="M12.8864 0.25H0.613636C0.274219 0.25 0 0.465458 0 0.732143V13.2679C0 13.5345 0.274219 13.75 0.613636 13.75H12.8864C13.2258 13.75 13.5 13.5345 13.5 13.2679V0.732143C13.5 0.465458 13.2258 0.25 12.8864 0.25ZM1.38068 1.33482H12.1193V4.46875H1.38068V1.33482ZM12.1193 8.56696H1.38068V5.43304H12.1193V8.56696ZM12.1193 12.6652H1.38068V9.53125H12.1193V12.6652ZM6.44318 2.41964H2.91477C2.8304 2.41964 2.76136 2.47388 2.76136 2.54018V3.26339C2.76136 3.32969 2.8304 3.38393 2.91477 3.38393H6.44318C6.52756 3.38393 6.59659 3.32969 6.59659 3.26339V2.54018C6.59659 2.47388 6.52756 2.41964 6.44318 2.41964ZM2.91477 7.48214H6.44318C6.52756 7.48214 6.59659 7.4279 6.59659 7.36161V6.63839C6.59659 6.5721 6.52756 6.51786 6.44318 6.51786H2.91477C2.8304 6.51786 2.76136 6.5721 2.76136 6.63839V7.36161C2.76136 7.4279 2.8304 7.48214 2.91477 7.48214ZM9.20455 11.1585C9.20455 11.3183 9.28536 11.4716 9.42921 11.5846C9.57306 11.6977 9.76816 11.7612 9.97159 11.7612C10.175 11.7612 10.3701 11.6977 10.514 11.5846C10.6578 11.4716 10.7386 11.3183 10.7386 11.1585C10.7386 10.9986 10.6578 10.8453 10.514 10.7323C10.3701 10.6193 10.175 10.5558 9.97159 10.5558C9.76816 10.5558 9.57306 10.6193 9.42921 10.7323C9.28536 10.8453 9.20455 10.9986 9.20455 11.1585Z" fill="currentColor"/>\n' +
            "</svg>\n",
        },
        children: [
          {
            path: "create-product",
            name: "create-product",
            component: CreateProductView,
            meta: {
              showInSideBar: false,
              verboseName: "Create Products",
              requiresAuth: true,
              allowed_roles: ["photographer", "admin", "owner", "client"],
              level: 1,
              icon:
                '<svg width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">\n' +
                '<path d="M12.8864 0.25H0.613636C0.274219 0.25 0 0.465458 0 0.732143V13.2679C0 13.5345 0.274219 13.75 0.613636 13.75H12.8864C13.2258 13.75 13.5 13.5345 13.5 13.2679V0.732143C13.5 0.465458 13.2258 0.25 12.8864 0.25ZM1.38068 1.33482H12.1193V4.46875H1.38068V1.33482ZM12.1193 8.56696H1.38068V5.43304H12.1193V8.56696ZM12.1193 12.6652H1.38068V9.53125H12.1193V12.6652ZM6.44318 2.41964H2.91477C2.8304 2.41964 2.76136 2.47388 2.76136 2.54018V3.26339C2.76136 3.32969 2.8304 3.38393 2.91477 3.38393H6.44318C6.52756 3.38393 6.59659 3.32969 6.59659 3.26339V2.54018C6.59659 2.47388 6.52756 2.41964 6.44318 2.41964ZM2.91477 7.48214H6.44318C6.52756 7.48214 6.59659 7.4279 6.59659 7.36161V6.63839C6.59659 6.5721 6.52756 6.51786 6.44318 6.51786H2.91477C2.8304 6.51786 2.76136 6.5721 2.76136 6.63839V7.36161C2.76136 7.4279 2.8304 7.48214 2.91477 7.48214ZM9.20455 11.1585C9.20455 11.3183 9.28536 11.4716 9.42921 11.5846C9.57306 11.6977 9.76816 11.7612 9.97159 11.7612C10.175 11.7612 10.3701 11.6977 10.514 11.5846C10.6578 11.4716 10.7386 11.3183 10.7386 11.1585C10.7386 10.9986 10.6578 10.8453 10.514 10.7323C10.3701 10.6193 10.175 10.5558 9.97159 10.5558C9.76816 10.5558 9.57306 10.6193 9.42921 10.7323C9.28536 10.8453 9.20455 10.9986 9.20455 11.1585Z" fill="currentColor"/>\n' +
                "</svg>\n",
            },
          },
          {
            path: "management/:productId",
            name: "product-management",
            component: ProductManagement,
            meta: {
              showInSideBar: false,
              verboseName: "Manage Products",
              requiresAuth: true,
              allowed_roles: ["photographer", "admin", "owner", "client"],
              level: 1,
              icon:
                '<svg width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">\n' +
                '<path d="M12.8864 0.25H0.613636C0.274219 0.25 0 0.465458 0 0.732143V13.2679C0 13.5345 0.274219 13.75 0.613636 13.75H12.8864C13.2258 13.75 13.5 13.5345 13.5 13.2679V0.732143C13.5 0.465458 13.2258 0.25 12.8864 0.25ZM1.38068 1.33482H12.1193V4.46875H1.38068V1.33482ZM12.1193 8.56696H1.38068V5.43304H12.1193V8.56696ZM12.1193 12.6652H1.38068V9.53125H12.1193V12.6652ZM6.44318 2.41964H2.91477C2.8304 2.41964 2.76136 2.47388 2.76136 2.54018V3.26339C2.76136 3.32969 2.8304 3.38393 2.91477 3.38393H6.44318C6.52756 3.38393 6.59659 3.32969 6.59659 3.26339V2.54018C6.59659 2.47388 6.52756 2.41964 6.44318 2.41964ZM2.91477 7.48214H6.44318C6.52756 7.48214 6.59659 7.4279 6.59659 7.36161V6.63839C6.59659 6.5721 6.52756 6.51786 6.44318 6.51786H2.91477C2.8304 6.51786 2.76136 6.5721 2.76136 6.63839V7.36161C2.76136 7.4279 2.8304 7.48214 2.91477 7.48214ZM9.20455 11.1585C9.20455 11.3183 9.28536 11.4716 9.42921 11.5846C9.57306 11.6977 9.76816 11.7612 9.97159 11.7612C10.175 11.7612 10.3701 11.6977 10.514 11.5846C10.6578 11.4716 10.7386 11.3183 10.7386 11.1585C10.7386 10.9986 10.6578 10.8453 10.514 10.7323C10.3701 10.6193 10.175 10.5558 9.97159 10.5558C9.76816 10.5558 9.57306 10.6193 9.42921 10.7323C9.28536 10.8453 9.20455 10.9986 9.20455 11.1585Z" fill="currentColor"/>\n' +
                "</svg>\n",
            },
            children: [
              {
                path: "add-inventory",
                name: "add-inventory",
                component: AddUpdateInventory,
                meta: {
                  showInSideBar: false,
                  verboseName: "Add Inventory",
                  requiresAuth: true,
                  allowed_roles: ["photographer", "admin", "owner", "client"],
                  level: 1,
                  icon:
                    '<svg width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">\n' +
                    '<path d="M12.8864 0.25H0.613636C0.274219 0.25 0 0.465458 0 0.732143V13.2679C0 13.5345 0.274219 13.75 0.613636 13.75H12.8864C13.2258 13.75 13.5 13.5345 13.5 13.2679V0.732143C13.5 0.465458 13.2258 0.25 12.8864 0.25ZM1.38068 1.33482H12.1193V4.46875H1.38068V1.33482ZM12.1193 8.56696H1.38068V5.43304H12.1193V8.56696ZM12.1193 12.6652H1.38068V9.53125H12.1193V12.6652ZM6.44318 2.41964H2.91477C2.8304 2.41964 2.76136 2.47388 2.76136 2.54018V3.26339C2.76136 3.32969 2.8304 3.38393 2.91477 3.38393H6.44318C6.52756 3.38393 6.59659 3.32969 6.59659 3.26339V2.54018C6.59659 2.47388 6.52756 2.41964 6.44318 2.41964ZM2.91477 7.48214H6.44318C6.52756 7.48214 6.59659 7.4279 6.59659 7.36161V6.63839C6.59659 6.5721 6.52756 6.51786 6.44318 6.51786H2.91477C2.8304 6.51786 2.76136 6.5721 2.76136 6.63839V7.36161C2.76136 7.4279 2.8304 7.48214 2.91477 7.48214ZM9.20455 11.1585C9.20455 11.3183 9.28536 11.4716 9.42921 11.5846C9.57306 11.6977 9.76816 11.7612 9.97159 11.7612C10.175 11.7612 10.3701 11.6977 10.514 11.5846C10.6578 11.4716 10.7386 11.3183 10.7386 11.1585C10.7386 10.9986 10.6578 10.8453 10.514 10.7323C10.3701 10.6193 10.175 10.5558 9.97159 10.5558C9.76816 10.5558 9.57306 10.6193 9.42921 10.7323C9.28536 10.8453 9.20455 10.9986 9.20455 11.1585Z" fill="currentColor"/>\n' +
                    "</svg>\n",
                },
              },
              {
                path: "edit-product",
                name: "edit-product",
                component: ProductForm,
                meta: {
                  showInSideBar: false,
                  verboseName: "Edit Product",
                  requiresAuth: true,
                  allowed_roles: ["photographer", "admin", "owner", "client"],
                  level: 1,
                  icon:
                    '<svg width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">\n' +
                    '<path d="M12.8864 0.25H0.613636C0.274219 0.25 0 0.465458 0 0.732143V13.2679C0 13.5345 0.274219 13.75 0.613636 13.75H12.8864C13.2258 13.75 13.5 13.5345 13.5 13.2679V0.732143C13.5 0.465458 13.2258 0.25 12.8864 0.25ZM1.38068 1.33482H12.1193V4.46875H1.38068V1.33482ZM12.1193 8.56696H1.38068V5.43304H12.1193V8.56696ZM12.1193 12.6652H1.38068V9.53125H12.1193V12.6652ZM6.44318 2.41964H2.91477C2.8304 2.41964 2.76136 2.47388 2.76136 2.54018V3.26339C2.76136 3.32969 2.8304 3.38393 2.91477 3.38393H6.44318C6.52756 3.38393 6.59659 3.32969 6.59659 3.26339V2.54018C6.59659 2.47388 6.52756 2.41964 6.44318 2.41964ZM2.91477 7.48214H6.44318C6.52756 7.48214 6.59659 7.4279 6.59659 7.36161V6.63839C6.59659 6.5721 6.52756 6.51786 6.44318 6.51786H2.91477C2.8304 6.51786 2.76136 6.5721 2.76136 6.63839V7.36161C2.76136 7.4279 2.8304 7.48214 2.91477 7.48214ZM9.20455 11.1585C9.20455 11.3183 9.28536 11.4716 9.42921 11.5846C9.57306 11.6977 9.76816 11.7612 9.97159 11.7612C10.175 11.7612 10.3701 11.6977 10.514 11.5846C10.6578 11.4716 10.7386 11.3183 10.7386 11.1585C10.7386 10.9986 10.6578 10.8453 10.514 10.7323C10.3701 10.6193 10.175 10.5558 9.97159 10.5558C9.76816 10.5558 9.57306 10.6193 9.42921 10.7323C9.28536 10.8453 9.20455 10.9986 9.20455 11.1585Z" fill="currentColor"/>\n' +
                    "</svg>\n",
                },
              },
              {
                path: "product-images",
                name: "product-images",
                component: UploadProductImages,
                meta: {
                  showInSideBar: false,
                  verboseName: "Add Inventory",
                  requiresAuth: true,
                  allowed_roles: ["photographer", "admin", "owner", "client"],
                  level: 1,
                  icon:
                    '<svg width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">\n' +
                    '<path d="M12.8864 0.25H0.613636C0.274219 0.25 0 0.465458 0 0.732143V13.2679C0 13.5345 0.274219 13.75 0.613636 13.75H12.8864C13.2258 13.75 13.5 13.5345 13.5 13.2679V0.732143C13.5 0.465458 13.2258 0.25 12.8864 0.25ZM1.38068 1.33482H12.1193V4.46875H1.38068V1.33482ZM12.1193 8.56696H1.38068V5.43304H12.1193V8.56696ZM12.1193 12.6652H1.38068V9.53125H12.1193V12.6652ZM6.44318 2.41964H2.91477C2.8304 2.41964 2.76136 2.47388 2.76136 2.54018V3.26339C2.76136 3.32969 2.8304 3.38393 2.91477 3.38393H6.44318C6.52756 3.38393 6.59659 3.32969 6.59659 3.26339V2.54018C6.59659 2.47388 6.52756 2.41964 6.44318 2.41964ZM2.91477 7.48214H6.44318C6.52756 7.48214 6.59659 7.4279 6.59659 7.36161V6.63839C6.59659 6.5721 6.52756 6.51786 6.44318 6.51786H2.91477C2.8304 6.51786 2.76136 6.5721 2.76136 6.63839V7.36161C2.76136 7.4279 2.8304 7.48214 2.91477 7.48214ZM9.20455 11.1585C9.20455 11.3183 9.28536 11.4716 9.42921 11.5846C9.57306 11.6977 9.76816 11.7612 9.97159 11.7612C10.175 11.7612 10.3701 11.6977 10.514 11.5846C10.6578 11.4716 10.7386 11.3183 10.7386 11.1585C10.7386 10.9986 10.6578 10.8453 10.514 10.7323C10.3701 10.6193 10.175 10.5558 9.97159 10.5558C9.76816 10.5558 9.57306 10.6193 9.42921 10.7323C9.28536 10.8453 9.20455 10.9986 9.20455 11.1585Z" fill="currentColor"/>\n' +
                    "</svg>\n",
                },
              },
            ],
          },
        ],
      },
      {
        path: "pos",
        name: "pos",
        component: PosPage,
        meta: {
          showInSideBar: false,
          verboseName: "Home",
          requiresAuth: true,
          allowed_roles: ["photographer", "admin", "owner", "client"],
          level: 1,
          icon:
            '<svg width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">\n' +
            '<path d="M12.8864 0.25H0.613636C0.274219 0.25 0 0.465458 0 0.732143V13.2679C0 13.5345 0.274219 13.75 0.613636 13.75H12.8864C13.2258 13.75 13.5 13.5345 13.5 13.2679V0.732143C13.5 0.465458 13.2258 0.25 12.8864 0.25ZM1.38068 1.33482H12.1193V4.46875H1.38068V1.33482ZM12.1193 8.56696H1.38068V5.43304H12.1193V8.56696ZM12.1193 12.6652H1.38068V9.53125H12.1193V12.6652ZM6.44318 2.41964H2.91477C2.8304 2.41964 2.76136 2.47388 2.76136 2.54018V3.26339C2.76136 3.32969 2.8304 3.38393 2.91477 3.38393H6.44318C6.52756 3.38393 6.59659 3.32969 6.59659 3.26339V2.54018C6.59659 2.47388 6.52756 2.41964 6.44318 2.41964ZM2.91477 7.48214H6.44318C6.52756 7.48214 6.59659 7.4279 6.59659 7.36161V6.63839C6.59659 6.5721 6.52756 6.51786 6.44318 6.51786H2.91477C2.8304 6.51786 2.76136 6.5721 2.76136 6.63839V7.36161C2.76136 7.4279 2.8304 7.48214 2.91477 7.48214ZM9.20455 11.1585C9.20455 11.3183 9.28536 11.4716 9.42921 11.5846C9.57306 11.6977 9.76816 11.7612 9.97159 11.7612C10.175 11.7612 10.3701 11.6977 10.514 11.5846C10.6578 11.4716 10.7386 11.3183 10.7386 11.1585C10.7386 10.9986 10.6578 10.8453 10.514 10.7323C10.3701 10.6193 10.175 10.5558 9.97159 10.5558C9.76816 10.5558 9.57306 10.6193 9.42921 10.7323C9.28536 10.8453 9.20455 10.9986 9.20455 11.1585Z" fill="currentColor"/>\n' +
            "</svg>\n",
        },
      },
      {
        path: "categories",
        name: "categories",
        component: CategoryList,
        meta: {
          showInSideBar: false,
          verboseName: "Categories",
          requiresAuth: true,
          allowed_roles: ["photographer", "admin", "owner", "client"],
          level: 1,
          icon:
            '<svg width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">\n' +
            '<path d="M12.8864 0.25H0.613636C0.274219 0.25 0 0.465458 0 0.732143V13.2679C0 13.5345 0.274219 13.75 0.613636 13.75H12.8864C13.2258 13.75 13.5 13.5345 13.5 13.2679V0.732143C13.5 0.465458 13.2258 0.25 12.8864 0.25ZM1.38068 1.33482H12.1193V4.46875H1.38068V1.33482ZM12.1193 8.56696H1.38068V5.43304H12.1193V8.56696ZM12.1193 12.6652H1.38068V9.53125H12.1193V12.6652ZM6.44318 2.41964H2.91477C2.8304 2.41964 2.76136 2.47388 2.76136 2.54018V3.26339C2.76136 3.32969 2.8304 3.38393 2.91477 3.38393H6.44318C6.52756 3.38393 6.59659 3.32969 6.59659 3.26339V2.54018C6.59659 2.47388 6.52756 2.41964 6.44318 2.41964ZM2.91477 7.48214H6.44318C6.52756 7.48214 6.59659 7.4279 6.59659 7.36161V6.63839C6.59659 6.5721 6.52756 6.51786 6.44318 6.51786H2.91477C2.8304 6.51786 2.76136 6.5721 2.76136 6.63839V7.36161C2.76136 7.4279 2.8304 7.48214 2.91477 7.48214ZM9.20455 11.1585C9.20455 11.3183 9.28536 11.4716 9.42921 11.5846C9.57306 11.6977 9.76816 11.7612 9.97159 11.7612C10.175 11.7612 10.3701 11.6977 10.514 11.5846C10.6578 11.4716 10.7386 11.3183 10.7386 11.1585C10.7386 10.9986 10.6578 10.8453 10.514 10.7323C10.3701 10.6193 10.175 10.5558 9.97159 10.5558C9.76816 10.5558 9.57306 10.6193 9.42921 10.7323C9.28536 10.8453 9.20455 10.9986 9.20455 11.1585Z" fill="currentColor"/>\n' +
            "</svg>\n",
        },
        children: [
          {
            path: "create-category",
            name: "create-category",
            component: CreateCategoryView,
            meta: {
              showInSideBar: false,
              verboseName: "Create Category",
              requiresAuth: true,
              allowed_roles: ["photographer", "admin", "owner", "client"],
              level: 1,
              icon:
                '<svg width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">\n' +
                '<path d="M12.8864 0.25H0.613636C0.274219 0.25 0 0.465458 0 0.732143V13.2679C0 13.5345 0.274219 13.75 0.613636 13.75H12.8864C13.2258 13.75 13.5 13.5345 13.5 13.2679V0.732143C13.5 0.465458 13.2258 0.25 12.8864 0.25ZM1.38068 1.33482H12.1193V4.46875H1.38068V1.33482ZM12.1193 8.56696H1.38068V5.43304H12.1193V8.56696ZM12.1193 12.6652H1.38068V9.53125H12.1193V12.6652ZM6.44318 2.41964H2.91477C2.8304 2.41964 2.76136 2.47388 2.76136 2.54018V3.26339C2.76136 3.32969 2.8304 3.38393 2.91477 3.38393H6.44318C6.52756 3.38393 6.59659 3.32969 6.59659 3.26339V2.54018C6.59659 2.47388 6.52756 2.41964 6.44318 2.41964ZM2.91477 7.48214H6.44318C6.52756 7.48214 6.59659 7.4279 6.59659 7.36161V6.63839C6.59659 6.5721 6.52756 6.51786 6.44318 6.51786H2.91477C2.8304 6.51786 2.76136 6.5721 2.76136 6.63839V7.36161C2.76136 7.4279 2.8304 7.48214 2.91477 7.48214ZM9.20455 11.1585C9.20455 11.3183 9.28536 11.4716 9.42921 11.5846C9.57306 11.6977 9.76816 11.7612 9.97159 11.7612C10.175 11.7612 10.3701 11.6977 10.514 11.5846C10.6578 11.4716 10.7386 11.3183 10.7386 11.1585C10.7386 10.9986 10.6578 10.8453 10.514 10.7323C10.3701 10.6193 10.175 10.5558 9.97159 10.5558C9.76816 10.5558 9.57306 10.6193 9.42921 10.7323C9.28536 10.8453 9.20455 10.9986 9.20455 11.1585Z" fill="currentColor"/>\n' +
                "</svg>\n",
            },
          },
          {
            path: "management/:categoryId",
            name: "category-management",
            component: CategoryManagement,
            meta: {
              showInSideBar: false,
              verboseName: "Manage Category",
              requiresAuth: true,
              allowed_roles: ["photographer", "admin", "owner", "client"],
              level: 1,
              icon:
                '<svg width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">\n' +
                '<path d="M12.8864 0.25H0.613636C0.274219 0.25 0 0.465458 0 0.732143V13.2679C0 13.5345 0.274219 13.75 0.613636 13.75H12.8864C13.2258 13.75 13.5 13.5345 13.5 13.2679V0.732143C13.5 0.465458 13.2258 0.25 12.8864 0.25ZM1.38068 1.33482H12.1193V4.46875H1.38068V1.33482ZM12.1193 8.56696H1.38068V5.43304H12.1193V8.56696ZM12.1193 12.6652H1.38068V9.53125H12.1193V12.6652ZM6.44318 2.41964H2.91477C2.8304 2.41964 2.76136 2.47388 2.76136 2.54018V3.26339C2.76136 3.32969 2.8304 3.38393 2.91477 3.38393H6.44318C6.52756 3.38393 6.59659 3.32969 6.59659 3.26339V2.54018C6.59659 2.47388 6.52756 2.41964 6.44318 2.41964ZM2.91477 7.48214H6.44318C6.52756 7.48214 6.59659 7.4279 6.59659 7.36161V6.63839C6.59659 6.5721 6.52756 6.51786 6.44318 6.51786H2.91477C2.8304 6.51786 2.76136 6.5721 2.76136 6.63839V7.36161C2.76136 7.4279 2.8304 7.48214 2.91477 7.48214ZM9.20455 11.1585C9.20455 11.3183 9.28536 11.4716 9.42921 11.5846C9.57306 11.6977 9.76816 11.7612 9.97159 11.7612C10.175 11.7612 10.3701 11.6977 10.514 11.5846C10.6578 11.4716 10.7386 11.3183 10.7386 11.1585C10.7386 10.9986 10.6578 10.8453 10.514 10.7323C10.3701 10.6193 10.175 10.5558 9.97159 10.5558C9.76816 10.5558 9.57306 10.6193 9.42921 10.7323C9.28536 10.8453 9.20455 10.9986 9.20455 11.1585Z" fill="currentColor"/>\n' +
                "</svg>\n",
            },
            children: [
              {
                path: "edit-category",
                name: "edit-category",
                component: EditCategoryView,
                meta: {
                  showInSideBar: false,
                  verboseName: "Edit Category",
                  requiresAuth: true,
                  allowed_roles: ["photographer", "admin", "owner", "client"],
                  level: 1,
                  icon:
                    '<svg width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">\n' +
                    '<path d="M12.8864 0.25H0.613636C0.274219 0.25 0 0.465458 0 0.732143V13.2679C0 13.5345 0.274219 13.75 0.613636 13.75H12.8864C13.2258 13.75 13.5 13.5345 13.5 13.2679V0.732143C13.5 0.465458 13.2258 0.25 12.8864 0.25ZM1.38068 1.33482H12.1193V4.46875H1.38068V1.33482ZM12.1193 8.56696H1.38068V5.43304H12.1193V8.56696ZM12.1193 12.6652H1.38068V9.53125H12.1193V12.6652ZM6.44318 2.41964H2.91477C2.8304 2.41964 2.76136 2.47388 2.76136 2.54018V3.26339C2.76136 3.32969 2.8304 3.38393 2.91477 3.38393H6.44318C6.52756 3.38393 6.59659 3.32969 6.59659 3.26339V2.54018C6.59659 2.47388 6.52756 2.41964 6.44318 2.41964ZM2.91477 7.48214H6.44318C6.52756 7.48214 6.59659 7.4279 6.59659 7.36161V6.63839C6.59659 6.5721 6.52756 6.51786 6.44318 6.51786H2.91477C2.8304 6.51786 2.76136 6.5721 2.76136 6.63839V7.36161C2.76136 7.4279 2.8304 7.48214 2.91477 7.48214ZM9.20455 11.1585C9.20455 11.3183 9.28536 11.4716 9.42921 11.5846C9.57306 11.6977 9.76816 11.7612 9.97159 11.7612C10.175 11.7612 10.3701 11.6977 10.514 11.5846C10.6578 11.4716 10.7386 11.3183 10.7386 11.1585C10.7386 10.9986 10.6578 10.8453 10.514 10.7323C10.3701 10.6193 10.175 10.5558 9.97159 10.5558C9.76816 10.5558 9.57306 10.6193 9.42921 10.7323C9.28536 10.8453 9.20455 10.9986 9.20455 11.1585Z" fill="currentColor"/>\n' +
                    "</svg>\n",
                },
              },
            ],
          },
        ],
      },
      {
        path: "blogs",
        name: "blogs",
        component: BlogList,
        meta: {
          showInSideBar: false,
          verboseName: "Blogs",
          requiresAuth: true,
          allowed_roles: ["photographer", "admin", "owner", "client"],
          level: 1,
          icon:
            '<svg width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">\n' +
            '<path d="M12.25 2.25H1.75C1.33579 2.25 1 2.58579 1 3V11C1 11.4142 1.33579 11.75 1.75 11.75H12.25C12.6642 11.75 13 11.4142 13 11V3C13 2.58579 12.6642 2.25 12.25 2.25ZM11.5 10.25H2.5V3.75H11.5V10.25ZM3.25 5.25H7.75V6.75H3.25V5.25ZM3.25 7.5H10.75V9H3.25V7.5Z" fill="currentColor"/>\n' +
            "</svg>\n",
        },
      },
      {
        path: "blogs/edit/:blogId",
        name: "edit-blog",
        component: EditBlogView,
        meta: {
          showInSideBar: false,
          verboseName: "Edit Blog",
          requiresAuth: true,
          allowed_roles: ["photographer", "admin", "owner", "client"],
          level: 1,
          icon:
            '<svg width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">\n' +
            '<path d="M12.25 2.25H1.75C1.33579 2.25 1 2.58579 1 3V11C1 11.4142 1.33579 11.75 1.75 11.75H12.25C12.6642 11.75 13 11.4142 13 11V3C13 2.58579 12.6642 2.25 12.25 2.25ZM11.5 10.25H2.5V3.75H11.5V10.25ZM3.25 5.25H7.75V6.75H3.25V5.25ZM3.25 7.5H10.75V9H3.25V7.5Z" fill="currentColor"/>\n' +
            "</svg>\n",
        },
      },
      {
        path: "blogs/categories",
        name: "blog-categories",
        component: BlogCategoryList,
        meta: {
          showInSideBar: false,
          verboseName: "Blog Categories",
          requiresAuth: true,
          allowed_roles: ["photographer", "admin", "owner", "client"],
          level: 1,
          icon:
            '<svg width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">\n' +
            '<path d="M12.25 2.25H1.75C1.33579 2.25 1 2.58579 1 3V11C1 11.4142 1.33579 11.75 1.75 11.75H12.25C12.6642 11.75 13 11.4142 13 11V3C13 2.58579 12.6642 2.25 12.25 2.25ZM11.5 10.25H2.5V3.75H11.5V10.25ZM3.25 5.25H7.75V6.75H3.25V5.25ZM3.25 7.5H10.75V9H3.25V7.5Z" fill="currentColor"/>\n' +
            "</svg>\n",
        },
      },
      {
        path: "orders",
        name: "orders",
        component: OrderList,
        meta: {
          showInSideBar: false,
          verboseName: "Orders",
          requiresAuth: true,
          allowed_roles: ["photographer", "admin", "owner", "client"],
          level: 1,
          icon:
            '<svg width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">\n' +
            '<path d="M12.8864 0.25H0.613636C0.274219 0.25 0 0.465458 0 0.732143V13.2679C0 13.5345 0.274219 13.75 0.613636 13.75H12.8864C13.2258 13.75 13.5 13.5345 13.5 13.2679V0.732143C13.5 0.465458 13.2258 0.25 12.8864 0.25ZM1.38068 1.33482H12.1193V4.46875H1.38068V1.33482ZM12.1193 8.56696H1.38068V5.43304H12.1193V8.56696ZM12.1193 12.6652H1.38068V9.53125H12.1193V12.6652ZM6.44318 2.41964H2.91477C2.8304 2.41964 2.76136 2.47388 2.76136 2.54018V3.26339C2.76136 3.32969 2.8304 3.38393 2.91477 3.38393H6.44318C6.52756 3.38393 6.59659 3.32969 6.59659 3.26339V2.54018C6.59659 2.47388 6.52756 2.41964 6.44318 2.41964ZM2.91477 7.48214H6.44318C6.52756 7.48214 6.59659 7.4279 6.59659 7.36161V6.63839C6.59659 6.5721 6.52756 6.51786 6.44318 6.51786H2.91477C2.8304 6.51786 2.76136 6.5721 2.76136 6.63839V7.36161C2.76136 7.4279 2.8304 7.48214 2.91477 7.48214ZM9.20455 11.1585C9.20455 11.3183 9.28536 11.4716 9.42921 11.5846C9.57306 11.6977 9.76816 11.7612 9.97159 11.7612C10.175 11.7612 10.3701 11.6977 10.514 11.5846C10.6578 11.4716 10.7386 11.3183 10.7386 11.1585C10.7386 10.9986 10.6578 10.8453 10.514 10.7323C10.3701 10.6193 10.175 10.5558 9.97159 10.5558C9.76816 10.5558 9.57306 10.6193 9.42921 10.7323C9.28536 10.8453 9.20455 10.9986 9.20455 11.1585Z" fill="currentColor"/>\n' +
            "</svg>\n",
        },
        children: [
          {
            path: "rider/:orderId",
            name: "order-rider",
            component: SelectRirder,
            meta: {
              showInSideBar: false,
              verboseName: "Manage Products",
              requiresAuth: true,
              allowed_roles: ["photographer", "admin", "owner", "client"],
              level: 1,
              icon:
                '<svg width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">\n' +
                '<path d="M12.8864 0.25H0.613636C0.274219 0.25 0 0.465458 0 0.732143V13.2679C0 13.5345 0.274219 13.75 0.613636 13.75H12.8864C13.2258 13.75 13.5 13.5345 13.5 13.2679V0.732143C13.5 0.465458 13.2258 0.25 12.8864 0.25ZM1.38068 1.33482H12.1193V4.46875H1.38068V1.33482ZM12.1193 8.56696H1.38068V5.43304H12.1193V8.56696ZM12.1193 12.6652H1.38068V9.53125H12.1193V12.6652ZM6.44318 2.41964H2.91477C2.8304 2.41964 2.76136 2.47388 2.76136 2.54018V3.26339C2.76136 3.32969 2.8304 3.38393 2.91477 3.38393H6.44318C6.52756 3.38393 6.59659 3.32969 6.59659 3.26339V2.54018C6.59659 2.47388 6.52756 2.41964 6.44318 2.41964ZM2.91477 7.48214H6.44318C6.52756 7.48214 6.59659 7.4279 6.59659 7.36161V6.63839C6.59659 6.5721 6.52756 6.51786 6.44318 6.51786H2.91477C2.8304 6.51786 2.76136 6.5721 2.76136 6.63839V7.36161C2.76136 7.4279 2.8304 7.48214 2.91477 7.48214ZM9.20455 11.1585C9.20455 11.3183 9.28536 11.4716 9.42921 11.5846C9.57306 11.6977 9.76816 11.7612 9.97159 11.7612C10.175 11.7612 10.3701 11.6977 10.514 11.5846C10.6578 11.4716 10.7386 11.3183 10.7386 11.1585C10.7386 10.9986 10.6578 10.8453 10.514 10.7323C10.3701 10.6193 10.175 10.5558 9.97159 10.5558C9.76816 10.5558 9.57306 10.6193 9.42921 10.7323C9.28536 10.8453 9.20455 10.9986 9.20455 11.1585Z" fill="currentColor"/>\n' +
                "</svg>\n",
            },
          },
          {
            path: "management/:orderId",
            name: "order-management",
            component: OrderManagement,
            meta: {
              showInSideBar: false,
              verboseName: "Manage Products",
              requiresAuth: true,
              allowed_roles: ["photographer", "admin", "owner", "client"],
              level: 1,
              icon:
                '<svg width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">\n' +
                '<path d="M12.8864 0.25H0.613636C0.274219 0.25 0 0.465458 0 0.732143V13.2679C0 13.5345 0.274219 13.75 0.613636 13.75H12.8864C13.2258 13.75 13.5 13.5345 13.5 13.2679V0.732143C13.5 0.465458 13.2258 0.25 12.8864 0.25ZM1.38068 1.33482H12.1193V4.46875H1.38068V1.33482ZM12.1193 8.56696H1.38068V5.43304H12.1193V8.56696ZM12.1193 12.6652H1.38068V9.53125H12.1193V12.6652ZM6.44318 2.41964H2.91477C2.8304 2.41964 2.76136 2.47388 2.76136 2.54018V3.26339C2.76136 3.32969 2.8304 3.38393 2.91477 3.38393H6.44318C6.52756 3.38393 6.59659 3.32969 6.59659 3.26339V2.54018C6.59659 2.47388 6.52756 2.41964 6.44318 2.41964ZM2.91477 7.48214H6.44318C6.52756 7.48214 6.59659 7.4279 6.59659 7.36161V6.63839C6.59659 6.5721 6.52756 6.51786 6.44318 6.51786H2.91477C2.8304 6.51786 2.76136 6.5721 2.76136 6.63839V7.36161C2.76136 7.4279 2.8304 7.48214 2.91477 7.48214ZM9.20455 11.1585C9.20455 11.3183 9.28536 11.4716 9.42921 11.5846C9.57306 11.6977 9.76816 11.7612 9.97159 11.7612C10.175 11.7612 10.3701 11.6977 10.514 11.5846C10.6578 11.4716 10.7386 11.3183 10.7386 11.1585C10.7386 10.9986 10.6578 10.8453 10.514 10.7323C10.3701 10.6193 10.175 10.5558 9.97159 10.5558C9.76816 10.5558 9.57306 10.6193 9.42921 10.7323C9.28536 10.8453 9.20455 10.9986 9.20455 11.1585Z" fill="currentColor"/>\n' +
                "</svg>\n",
            },
            children: [
              {
                path: "order-rider",
                name: "order-rider",
                component: SelectRirder,
                meta: {
                  showInSideBar: false,
                  verboseName: "Order Rider",
                  requiresAuth: true,
                  allowed_roles: ["photographer", "admin", "owner", "client"],
                  level: 1,
                  icon:
                    '<svg width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">\n' +
                    '<path d="M12.8864 0.25H0.613636C0.274219 0.25 0 0.465458 0 0.732143V13.2679C0 13.5345 0.274219 13.75 0.613636 13.75H12.8864C13.2258 13.75 13.5 13.5345 13.5 13.2679V0.732143C13.5 0.465458 13.2258 0.25 12.8864 0.25ZM1.38068 1.33482H12.1193V4.46875H1.38068V1.33482ZM12.1193 8.56696H1.38068V5.43304H12.1193V8.56696ZM12.1193 12.6652H1.38068V9.53125H12.1193V12.6652ZM6.44318 2.41964H2.91477C2.8304 2.41964 2.76136 2.47388 2.76136 2.54018V3.26339C2.76136 3.32969 2.8304 3.38393 2.91477 3.38393H6.44318C6.52756 3.38393 6.59659 3.32969 6.59659 3.26339V2.54018C6.59659 2.47388 6.52756 2.41964 6.44318 2.41964ZM2.91477 7.48214H6.44318C6.52756 7.48214 6.59659 7.4279 6.59659 7.36161V6.63839C6.59659 6.5721 6.52756 6.51786 6.44318 6.51786H2.91477C2.8304 6.51786 2.76136 6.5721 2.76136 6.63839V7.36161C2.76136 7.4279 2.8304 7.48214 2.91477 7.48214ZM9.20455 11.1585C9.20455 11.3183 9.28536 11.4716 9.42921 11.5846C9.57306 11.6977 9.76816 11.7612 9.97159 11.7612C10.175 11.7612 10.3701 11.6977 10.514 11.5846C10.6578 11.4716 10.7386 11.3183 10.7386 11.1585C10.7386 10.9986 10.6578 10.8453 10.514 10.7323C10.3701 10.6193 10.175 10.5558 9.97159 10.5558C9.76816 10.5558 9.57306 10.6193 9.42921 10.7323C9.28536 10.8453 9.20455 10.9986 9.20455 11.1585Z" fill="currentColor"/>\n' +
                    "</svg>\n",
                },
              },
            ],
          },
        ],
      },
      {
        path: "shops",
        name: "shops",
        component: VendorList,
        meta: {
          showInSideBar: false,
          verboseName: "Shops",
          requiresAuth: true,
          allowed_roles: [
            "photographer",
            "admin",
            "owner",
            "client",
            "business_owner",
          ],
          level: 1,
          icon:
            '<svg width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">\n' +
            '<path d="M12.8864 0.25H0.613636C0.274219 0.25 0 0.465458 0 0.732143V13.2679C0 13.5345 0.274219 13.75 0.613636 13.75H12.8864C13.2258 13.75 13.5 13.5345 13.5 13.2679V0.732143C13.5 0.465458 13.2258 0.25 12.8864 0.25ZM1.38068 1.33482H12.1193V4.46875H1.38068V1.33482ZM12.1193 8.56696H1.38068V5.43304H12.1193V8.56696ZM12.1193 12.6652H1.38068V9.53125H12.1193V12.6652ZM6.44318 2.41964H2.91477C2.8304 2.41964 2.76136 2.47388 2.76136 2.54018V3.26339C2.76136 3.32969 2.8304 3.38393 2.91477 3.38393H6.44318C6.52756 3.38393 6.59659 3.32969 6.59659 3.26339V2.54018C6.59659 2.47388 6.52756 2.41964 6.44318 2.41964ZM2.91477 7.48214H6.44318C6.52756 7.48214 6.59659 7.4279 6.59659 7.36161V6.63839C6.59659 6.5721 6.52756 6.51786 6.44318 6.51786H2.91477C2.8304 6.51786 2.76136 6.5721 2.76136 6.63839V7.36161C2.76136 7.4279 2.8304 7.48214 2.91477 7.48214ZM9.20455 11.1585C9.20455 11.3183 9.28536 11.4716 9.42921 11.5846C9.57306 11.6977 9.76816 11.7612 9.97159 11.7612C10.175 11.7612 10.3701 11.6977 10.514 11.5846C10.6578 11.4716 10.7386 11.3183 10.7386 11.1585C10.7386 10.9986 10.6578 10.8453 10.514 10.7323C10.3701 10.6193 10.175 10.5558 9.97159 10.5558C9.76816 10.5558 9.57306 10.6193 9.42921 10.7323C9.28536 10.8453 9.20455 10.9986 9.20455 11.1585Z" fill="currentColor"/>\n' +
            "</svg>\n",
        },
        children: [
          {
            path: "create-vendor",
            name: "create-vendor",
            component: CreateVendorView,
            meta: {
              showInSideBar: false,
              verboseName: "Create Shops",
              requiresAuth: true,
              allowed_roles: [],
              level: 1,
              icon:
                '<svg width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">\n' +
                '<path d="M12.8864 0.25H0.613636C0.274219 0.25 0 0.465458 0 0.732143V13.2679C0 13.5345 0.274219 13.75 0.613636 13.75H12.8864C13.2258 13.75 13.5 13.5345 13.5 13.2679V0.732143C13.5 0.465458 13.2258 0.25 12.8864 0.25ZM1.38068 1.33482H12.1193V4.46875H1.38068V1.33482ZM12.1193 8.56696H1.38068V5.43304H12.1193V8.56696ZM12.1193 12.6652H1.38068V9.53125H12.1193V12.6652ZM6.44318 2.41964H2.91477C2.8304 2.41964 2.76136 2.47388 2.76136 2.54018V3.26339C2.76136 3.32969 2.8304 3.38393 2.91477 3.38393H6.44318C6.52756 3.38393 6.59659 3.32969 6.59659 3.26339V2.54018C6.59659 2.47388 6.52756 2.41964 6.44318 2.41964ZM2.91477 7.48214H6.44318C6.52756 7.48214 6.59659 7.4279 6.59659 7.36161V6.63839C6.59659 6.5721 6.52756 6.51786 6.44318 6.51786H2.91477C2.8304 6.51786 2.76136 6.5721 2.76136 6.63839V7.36161C2.76136 7.4279 2.8304 7.48214 2.91477 7.48214ZM9.20455 11.1585C9.20455 11.3183 9.28536 11.4716 9.42921 11.5846C9.57306 11.6977 9.76816 11.7612 9.97159 11.7612C10.175 11.7612 10.3701 11.6977 10.514 11.5846C10.6578 11.4716 10.7386 11.3183 10.7386 11.1585C10.7386 10.9986 10.6578 10.8453 10.514 10.7323C10.3701 10.6193 10.175 10.5558 9.97159 10.5558C9.76816 10.5558 9.57306 10.6193 9.42921 10.7323C9.28536 10.8453 9.20455 10.9986 9.20455 11.1585Z" fill="currentColor"/>\n' +
                "</svg>\n",
            },
          },
          {
            path: "management/:vendorId",
            name: "vendor-management",
            component: VendorManagement,
            meta: {
              showInSideBar: false,
              verboseName: "Manage Vendors",
              requiresAuth: true,
              allowed_roles: [],
              level: 1,
              icon:
                '<svg width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">\n' +
                '<path d="M12.8864 0.25H0.613636C0.274219 0.25 0 0.465458 0 0.732143V13.2679C0 13.5345 0.274219 13.75 0.613636 13.75H12.8864C13.2258 13.75 13.5 13.5345 13.5 13.2679V0.732143C13.5 0.465458 13.2258 0.25 12.8864 0.25ZM1.38068 1.33482H12.1193V4.46875H1.38068V1.33482ZM12.1193 8.56696H1.38068V5.43304H12.1193V8.56696ZM12.1193 12.6652H1.38068V9.53125H12.1193V12.6652ZM6.44318 2.41964H2.91477C2.8304 2.41964 2.76136 2.47388 2.76136 2.54018V3.26339C2.76136 3.32969 2.8304 3.38393 2.91477 3.38393H6.44318C6.52756 3.38393 6.59659 3.32969 6.59659 3.26339V2.54018C6.59659 2.47388 6.52756 2.41964 6.44318 2.41964ZM2.91477 7.48214H6.44318C6.52756 7.48214 6.59659 7.4279 6.59659 7.36161V6.63839C6.59659 6.5721 6.52756 6.51786 6.44318 6.51786H2.91477C2.8304 6.51786 2.76136 6.5721 2.76136 6.63839V7.36161C2.76136 7.4279 2.8304 7.48214 2.91477 7.48214ZM9.20455 11.1585C9.20455 11.3183 9.28536 11.4716 9.42921 11.5846C9.57306 11.6977 9.76816 11.7612 9.97159 11.7612C10.175 11.7612 10.3701 11.6977 10.514 11.5846C10.6578 11.4716 10.7386 11.3183 10.7386 11.1585C10.7386 10.9986 10.6578 10.8453 10.514 10.7323C10.3701 10.6193 10.175 10.5558 9.97159 10.5558C9.76816 10.5558 9.57306 10.6193 9.42921 10.7323C9.28536 10.8453 9.20455 10.9986 9.20455 11.1585Z" fill="currentColor"/>\n' +
                "</svg>\n",
            },
            children: [
              {
                path: "edit-vendor",
                name: "edit-vendor",
                component: VendorForm,
                meta: {
                  showInSideBar: false,
                  verboseName: "Edit Vendor",
                  requiresAuth: true,
                  allowed_roles: [],
                  level: 1,
                  icon:
                    '<svg width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">\n' +
                    '<path d="M12.8864 0.25H0.613636C0.274219 0.25 0 0.465458 0 0.732143V13.2679C0 13.5345 0.274219 13.75 0.613636 13.75H12.8864C13.2258 13.75 13.5 13.5345 13.5 13.2679V0.732143C13.5 0.465458 13.2258 0.25 12.8864 0.25ZM1.38068 1.33482H12.1193V4.46875H1.38068V1.33482ZM12.1193 8.56696H1.38068V5.43304H12.1193V8.56696ZM12.1193 12.6652H1.38068V9.53125H12.1193V12.6652ZM6.44318 2.41964H2.91477C2.8304 2.41964 2.76136 2.47388 2.76136 2.54018V3.26339C2.76136 3.32969 2.8304 3.38393 2.91477 3.38393H6.44318C6.52756 3.38393 6.59659 3.32969 6.59659 3.26339V2.54018C6.59659 2.47388 6.52756 2.41964 6.44318 2.41964ZM2.91477 7.48214H6.44318C6.52756 7.48214 6.59659 7.4279 6.59659 7.36161V6.63839C6.59659 6.5721 6.52756 6.51786 6.44318 6.51786H2.91477C2.8304 6.51786 2.76136 6.5721 2.76136 6.63839V7.36161C2.76136 7.4279 2.8304 7.48214 2.91477 7.48214ZM9.20455 11.1585C9.20455 11.3183 9.28536 11.4716 9.42921 11.5846C9.57306 11.6977 9.76816 11.7612 9.97159 11.7612C10.175 11.7612 10.3701 11.6977 10.514 11.5846C10.6578 11.4716 10.7386 11.3183 10.7386 11.1585C10.7386 10.9986 10.6578 10.8453 10.514 10.7323C10.3701 10.6193 10.175 10.5558 9.97159 10.5558C9.76816 10.5558 9.57306 10.6193 9.42921 10.7323C9.28536 10.8453 9.20455 10.9986 9.20455 11.1585Z" fill="currentColor"/>\n' +
                    "</svg>\n",
                },
              },
              {
                path: "add-members",
                name: "add-members",
                component: MemberShipForm,
                meta: {
                  showInSideBar: false,
                  verboseName: "Vendor Membership",
                  requiresAuth: true,
                  allowed_roles: ["photographer", "admin", "owner", "client"],
                  level: 1,
                  icon:
                    '<svg width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">\n' +
                    '<path d="M12.8864 0.25H0.613636C0.274219 0.25 0 0.465458 0 0.732143V13.2679C0 13.5345 0.274219 13.75 0.613636 13.75H12.8864C13.2258 13.75 13.5 13.5345 13.5 13.2679V0.732143C13.5 0.465458 13.2258 0.25 12.8864 0.25ZM1.38068 1.33482H12.1193V4.46875H1.38068V1.33482ZM12.1193 8.56696H1.38068V5.43304H12.1193V8.56696ZM12.1193 12.6652H1.38068V9.53125H12.1193V12.6652ZM6.44318 2.41964H2.91477C2.8304 2.41964 2.76136 2.47388 2.76136 2.54018V3.26339C2.76136 3.32969 2.8304 3.38393 2.91477 3.38393H6.44318C6.52756 3.38393 6.59659 3.32969 6.59659 3.26339V2.54018C6.59659 2.47388 6.52756 2.41964 6.44318 2.41964ZM2.91477 7.48214H6.44318C6.52756 7.48214 6.59659 7.4279 6.59659 7.36161V6.63839C6.59659 6.5721 6.52756 6.51786 6.44318 6.51786H2.91477C2.8304 6.51786 2.76136 6.5721 2.76136 6.63839V7.36161C2.76136 7.4279 2.8304 7.48214 2.91477 7.48214ZM9.20455 11.1585C9.20455 11.3183 9.28536 11.4716 9.42921 11.5846C9.57306 11.6977 9.76816 11.7612 9.97159 11.7612C10.175 11.7612 10.3701 11.6977 10.514 11.5846C10.6578 11.4716 10.7386 11.3183 10.7386 11.1585C10.7386 10.9986 10.6578 10.8453 10.514 10.7323C10.3701 10.6193 10.175 10.5558 9.97159 10.5558C9.76816 10.5558 9.57306 10.6193 9.42921 10.7323C9.28536 10.8453 9.20455 10.9986 9.20455 11.1585Z" fill="currentColor"/>\n' +
                    "</svg>\n",
                },
              },
              {
                path: "mini-register-rider",
                name: "mini-register-rider",
                component: RiderForm,
                meta: {
                  showInSideBar: false,
                  verboseName: "Vendor Membership",
                  requiresAuth: true,
                  allowed_roles: ["photographer", "admin", "owner", "client"],
                  level: 1,
                  icon:
                    '<svg width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">\n' +
                    '<path d="M12.8864 0.25H0.613636C0.274219 0.25 0 0.465458 0 0.732143V13.2679C0 13.5345 0.274219 13.75 0.613636 13.75H12.8864C13.2258 13.75 13.5 13.5345 13.5 13.2679V0.732143C13.5 0.465458 13.2258 0.25 12.8864 0.25ZM1.38068 1.33482H12.1193V4.46875H1.38068V1.33482ZM12.1193 8.56696H1.38068V5.43304H12.1193V8.56696ZM12.1193 12.6652H1.38068V9.53125H12.1193V12.6652ZM6.44318 2.41964H2.91477C2.8304 2.41964 2.76136 2.47388 2.76136 2.54018V3.26339C2.76136 3.32969 2.8304 3.38393 2.91477 3.38393H6.44318C6.52756 3.38393 6.59659 3.32969 6.59659 3.26339V2.54018C6.59659 2.47388 6.52756 2.41964 6.44318 2.41964ZM2.91477 7.48214H6.44318C6.52756 7.48214 6.59659 7.4279 6.59659 7.36161V6.63839C6.59659 6.5721 6.52756 6.51786 6.44318 6.51786H2.91477C2.8304 6.51786 2.76136 6.5721 2.76136 6.63839V7.36161C2.76136 7.4279 2.8304 7.48214 2.91477 7.48214ZM9.20455 11.1585C9.20455 11.3183 9.28536 11.4716 9.42921 11.5846C9.57306 11.6977 9.76816 11.7612 9.97159 11.7612C10.175 11.7612 10.3701 11.6977 10.514 11.5846C10.6578 11.4716 10.7386 11.3183 10.7386 11.1585C10.7386 10.9986 10.6578 10.8453 10.514 10.7323C10.3701 10.6193 10.175 10.5558 9.97159 10.5558C9.76816 10.5558 9.57306 10.6193 9.42921 10.7323C9.28536 10.8453 9.20455 10.9986 9.20455 11.1585Z" fill="currentColor"/>\n' +
                    "</svg>\n",
                },
              },
              {
                path: "mini-list-rider",
                name: "mini-list-rider",
                component: MiniRiderList,
                meta: {
                  showInSideBar: false,
                  verboseName: "Vendor Membership",
                  requiresAuth: true,
                  allowed_roles: ["photographer", "admin", "owner", "client"],
                  level: 1,
                  icon:
                    '<svg width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">\n' +
                    '<path d="M12.8864 0.25H0.613636C0.274219 0.25 0 0.465458 0 0.732143V13.2679C0 13.5345 0.274219 13.75 0.613636 13.75H12.8864C13.2258 13.75 13.5 13.5345 13.5 13.2679V0.732143C13.5 0.465458 13.2258 0.25 12.8864 0.25ZM1.38068 1.33482H12.1193V4.46875H1.38068V1.33482ZM12.1193 8.56696H1.38068V5.43304H12.1193V8.56696ZM12.1193 12.6652H1.38068V9.53125H12.1193V12.6652ZM6.44318 2.41964H2.91477C2.8304 2.41964 2.76136 2.47388 2.76136 2.54018V3.26339C2.76136 3.32969 2.8304 3.38393 2.91477 3.38393H6.44318C6.52756 3.38393 6.59659 3.32969 6.59659 3.26339V2.54018C6.59659 2.47388 6.52756 2.41964 6.44318 2.41964ZM2.91477 7.48214H6.44318C6.52756 7.48214 6.59659 7.4279 6.59659 7.36161V6.63839C6.59659 6.5721 6.52756 6.51786 6.44318 6.51786H2.91477C2.8304 6.51786 2.76136 6.5721 2.76136 6.63839V7.36161C2.76136 7.4279 2.8304 7.48214 2.91477 7.48214ZM9.20455 11.1585C9.20455 11.3183 9.28536 11.4716 9.42921 11.5846C9.57306 11.6977 9.76816 11.7612 9.97159 11.7612C10.175 11.7612 10.3701 11.6977 10.514 11.5846C10.6578 11.4716 10.7386 11.3183 10.7386 11.1585C10.7386 10.9986 10.6578 10.8453 10.514 10.7323C10.3701 10.6193 10.175 10.5558 9.97159 10.5558C9.76816 10.5558 9.57306 10.6193 9.42921 10.7323C9.28536 10.8453 9.20455 10.9986 9.20455 11.1585Z" fill="currentColor"/>\n' +
                    "</svg>\n",
                },
              },
              {
                path: "list-members",
                name: "list-members",
                component: VendorMembers,
                meta: {
                  showInSideBar: false,
                  verboseName: "Vendor Membership",
                  requiresAuth: true,
                  allowed_roles: ["photographer", "admin", "owner", "client"],
                  level: 1,
                  icon:
                    '<svg width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">\n' +
                    '<path d="M12.8864 0.25H0.613636C0.274219 0.25 0 0.465458 0 0.732143V13.2679C0 13.5345 0.274219 13.75 0.613636 13.75H12.8864C13.2258 13.75 13.5 13.5345 13.5 13.2679V0.732143C13.5 0.465458 13.2258 0.25 12.8864 0.25ZM1.38068 1.33482H12.1193V4.46875H1.38068V1.33482ZM12.1193 8.56696H1.38068V5.43304H12.1193V8.56696ZM12.1193 12.6652H1.38068V9.53125H12.1193V12.6652ZM6.44318 2.41964H2.91477C2.8304 2.41964 2.76136 2.47388 2.76136 2.54018V3.26339C2.76136 3.32969 2.8304 3.38393 2.91477 3.38393H6.44318C6.52756 3.38393 6.59659 3.32969 6.59659 3.26339V2.54018C6.59659 2.47388 6.52756 2.41964 6.44318 2.41964ZM2.91477 7.48214H6.44318C6.52756 7.48214 6.59659 7.4279 6.59659 7.36161V6.63839C6.59659 6.5721 6.52756 6.51786 6.44318 6.51786H2.91477C2.8304 6.51786 2.76136 6.5721 2.76136 6.63839V7.36161C2.76136 7.4279 2.8304 7.48214 2.91477 7.48214ZM9.20455 11.1585C9.20455 11.3183 9.28536 11.4716 9.42921 11.5846C9.57306 11.6977 9.76816 11.7612 9.97159 11.7612C10.175 11.7612 10.3701 11.6977 10.514 11.5846C10.6578 11.4716 10.7386 11.3183 10.7386 11.1585C10.7386 10.9986 10.6578 10.8453 10.514 10.7323C10.3701 10.6193 10.175 10.5558 9.97159 10.5558C9.76816 10.5558 9.57306 10.6193 9.42921 10.7323C9.28536 10.8453 9.20455 10.9986 9.20455 11.1585Z" fill="currentColor"/>\n' +
                    "</svg>\n",
                },
              },
            ],
          },
        ],
      },
      {
        path: "riders",
        name: "riders",
        component: MiniRiderList,
        meta: {
          showInSideBar: false,
          verboseName: "Riders",
          requiresAuth: true,
          allowed_roles: [],
          level: 1,
          icon:
            '<svg width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">\n' +
            '<path d="M12.8864 0.25H0.613636C0.274219 0.25 0 0.465458 0 0.732143V13.2679C0 13.5345 0.274219 13.75 0.613636 13.75H12.8864C13.2258 13.75 13.5 13.5345 13.5 13.2679V0.732143C13.5 0.465458 13.2258 0.25 12.8864 0.25ZM1.38068 1.33482H12.1193V4.46875H1.38068V1.33482ZM12.1193 8.56696H1.38068V5.43304H12.1193V8.56696ZM12.1193 12.6652H1.38068V9.53125H12.1193V12.6652ZM6.44318 2.41964H2.91477C2.8304 2.41964 2.76136 2.47388 2.76136 2.54018V3.26339C2.76136 3.32969 2.8304 3.38393 2.91477 3.38393H6.44318C6.52756 3.38393 6.59659 3.32969 6.59659 3.26339V2.54018C6.59659 2.47388 6.52756 2.41964 6.44318 2.41964ZM2.91477 7.48214H6.44318C6.52756 7.48214 6.59659 7.4279 6.59659 7.36161V6.63839C6.59659 6.5721 6.52756 6.51786 6.44318 6.51786H2.91477C2.8304 6.51786 2.76136 6.5721 2.76136 6.63839V7.36161C2.76136 7.4279 2.8304 7.48214 2.91477 7.48214ZM9.20455 11.1585C9.20455 11.3183 9.28536 11.4716 9.42921 11.5846C9.57306 11.6977 9.76816 11.7612 9.97159 11.7612C10.175 11.7612 10.3701 11.6977 10.514 11.5846C10.6578 11.4716 10.7386 11.3183 10.7386 11.1585C10.7386 10.9986 10.6578 10.8453 10.514 10.7323C10.3701 10.6193 10.175 10.5558 9.97159 10.5558C9.76816 10.5558 9.57306 10.6193 9.42921 10.7323C9.28536 10.8453 9.20455 10.9986 9.20455 11.1585Z" fill="currentColor"/>\n' +
            "</svg>\n",
        },
        children: [
          {
            path: "register-rider",
            name: "register-rider",
            component: RegisterRiderView,
            meta: {
              showInSideBar: false,
              verboseName: "Riders",
              requiresAuth: true,
              allowed_roles: [],
              level: 1,
              icon:
                '<svg width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">\n' +
                '<path d="M12.8864 0.25H0.613636C0.274219 0.25 0 0.465458 0 0.732143V13.2679C0 13.5345 0.274219 13.75 0.613636 13.75H12.8864C13.2258 13.75 13.5 13.5345 13.5 13.2679V0.732143C13.5 0.465458 13.2258 0.25 12.8864 0.25ZM1.38068 1.33482H12.1193V4.46875H1.38068V1.33482ZM12.1193 8.56696H1.38068V5.43304H12.1193V8.56696ZM12.1193 12.6652H1.38068V9.53125H12.1193V12.6652ZM6.44318 2.41964H2.91477C2.8304 2.41964 2.76136 2.47388 2.76136 2.54018V3.26339C2.76136 3.32969 2.8304 3.38393 2.91477 3.38393H6.44318C6.52756 3.38393 6.59659 3.32969 6.59659 3.26339V2.54018C6.59659 2.47388 6.52756 2.41964 6.44318 2.41964ZM2.91477 7.48214H6.44318C6.52756 7.48214 6.59659 7.4279 6.59659 7.36161V6.63839C6.59659 6.5721 6.52756 6.51786 6.44318 6.51786H2.91477C2.8304 6.51786 2.76136 6.5721 2.76136 6.63839V7.36161C2.76136 7.4279 2.8304 7.48214 2.91477 7.48214ZM9.20455 11.1585C9.20455 11.3183 9.28536 11.4716 9.42921 11.5846C9.57306 11.6977 9.76816 11.7612 9.97159 11.7612C10.175 11.7612 10.3701 11.6977 10.514 11.5846C10.6578 11.4716 10.7386 11.3183 10.7386 11.1585C10.7386 10.9986 10.6578 10.8453 10.514 10.7323C10.3701 10.6193 10.175 10.5558 9.97159 10.5558C9.76816 10.5558 9.57306 10.6193 9.42921 10.7323C9.28536 10.8453 9.20455 10.9986 9.20455 11.1585Z" fill="currentColor"/>\n' +
                "</svg>\n",
            },
          },
        ],
      },
      {
        path: "wallet",
        name: "wallet",
        component: WalletIndex,
        meta: {
          showInSideBar: true,
          verboseName: "Wallet",
          level: 1,
          icon:
            '<svg width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">\n' +
            '<path d="M12.8864 0.25H0.613636C0.274219 0.25 0 0.465458 0 0.732143V13.2679C0 13.5345 0.274219 13.75 0.613636 13.75H12.8864C13.2258 13.75 13.5 13.5345 13.5 13.2679V0.732143C13.5 0.465458 13.2258 0.25 12.8864 0.25ZM1.38068 1.33482H12.1193V4.46875H1.38068V1.33482ZM12.1193 8.56696H1.38068V5.43304H12.1193V8.56696ZM12.1193 12.6652H1.38068V9.53125H12.1193V12.6652ZM6.44318 2.41964H2.91477C2.8304 2.41964 2.76136 2.47388 2.76136 2.54018V3.26339C2.76136 3.32969 2.8304 3.38393 2.91477 3.38393H6.44318C6.52756 3.38393 6.59659 3.32969 6.59659 3.26339V2.54018C6.59659 2.47388 6.52756 2.41964 6.44318 2.41964ZM2.91477 7.48214H6.44318C6.52756 7.48214 6.59659 7.4279 6.59659 7.36161V6.63839C6.59659 6.5721 6.52756 6.51786 6.44318 6.51786H2.91477C2.8304 6.51786 2.76136 6.5721 2.76136 6.63839V7.36161C2.76136 7.4279 2.8304 7.48214 2.91477 7.48214ZM9.20455 11.1585C9.20455 11.3183 9.28536 11.4716 9.42921 11.5846C9.57306 11.6977 9.76816 11.7612 9.97159 11.7612C10.175 11.7612 10.3701 11.6977 10.514 11.5846C10.6578 11.4716 10.7386 11.3183 10.7386 11.1585C10.7386 10.9986 10.6578 10.8453 10.514 10.7323C10.3701 10.6193 10.175 10.5558 9.97159 10.5558C9.76816 10.5558 9.57306 10.6193 9.42921 10.7323C9.28536 10.8453 9.20455 10.9986 9.20455 11.1585Z" fill="currentColor"/>\n' +
            "</svg>\n",
          requiresAuth: false,
          allowed_roles: ["photographer", "admin", "owner", "client"],
        },
        children: [
          {
            path: "payments",
            name: "payments",
            component: payments,
            meta: {
              showInSideBar: true,
              verboseName: "Payments",
              level: 1,
              icon:
                '<svg width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">\n' +
                '<path d="M12.8864 0.25H0.613636C0.274219 0.25 0 0.465458 0 0.732143V13.2679C0 13.5345 0.274219 13.75 0.613636 13.75H12.8864C13.2258 13.75 13.5 13.5345 13.5 13.2679V0.732143C13.5 0.465458 13.2258 0.25 12.8864 0.25ZM1.38068 1.33482H12.1193V4.46875H1.38068V1.33482ZM12.1193 8.56696H1.38068V5.43304H12.1193V8.56696ZM12.1193 12.6652H1.38068V9.53125H12.1193V12.6652ZM6.44318 2.41964H2.91477C2.8304 2.41964 2.76136 2.47388 2.76136 2.54018V3.26339C2.76136 3.32969 2.8304 3.38393 2.91477 3.38393H6.44318C6.52756 3.38393 6.59659 3.32969 6.59659 3.26339V2.54018C6.59659 2.47388 6.52756 2.41964 6.44318 2.41964ZM2.91477 7.48214H6.44318C6.52756 7.48214 6.59659 7.4279 6.59659 7.36161V6.63839C6.59659 6.5721 6.52756 6.51786 6.44318 6.51786H2.91477C2.8304 6.51786 2.76136 6.5721 2.76136 6.63839V7.36161C2.76136 7.4279 2.8304 7.48214 2.91477 7.48214ZM9.20455 11.1585C9.20455 11.3183 9.28536 11.4716 9.42921 11.5846C9.57306 11.6977 9.76816 11.7612 9.97159 11.7612C10.175 11.7612 10.3701 11.6977 10.514 11.5846C10.6578 11.4716 10.7386 11.3183 10.7386 11.1585C10.7386 10.9986 10.6578 10.8453 10.514 10.7323C10.3701 10.6193 10.175 10.5558 9.97159 10.5558C9.76816 10.5558 9.57306 10.6193 9.42921 10.7323C9.28536 10.8453 9.20455 10.9986 9.20455 11.1585Z" fill="currentColor"/>\n' +
                "</svg>\n",
              requiresAuth: false,
              allowed_roles: ["photographer", "admin", "owner", "client"],
            },
          },
          {
            path: "client-payments",
            name: "client-payments",
            component: CustomerPayments,
            meta: {
              showInSideBar: true,
              verboseName: "Payments",
              level: 1,
              icon:
                '<svg width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">\n' +
                '<path d="M12.8864 0.25H0.613636C0.274219 0.25 0 0.465458 0 0.732143V13.2679C0 13.5345 0.274219 13.75 0.613636 13.75H12.8864C13.2258 13.75 13.5 13.5345 13.5 13.2679V0.732143C13.5 0.465458 13.2258 0.25 12.8864 0.25ZM1.38068 1.33482H12.1193V4.46875H1.38068V1.33482ZM12.1193 8.56696H1.38068V5.43304H12.1193V8.56696ZM12.1193 12.6652H1.38068V9.53125H12.1193V12.6652ZM6.44318 2.41964H2.91477C2.8304 2.41964 2.76136 2.47388 2.76136 2.54018V3.26339C2.76136 3.32969 2.8304 3.38393 2.91477 3.38393H6.44318C6.52756 3.38393 6.59659 3.32969 6.59659 3.26339V2.54018C6.59659 2.47388 6.52756 2.41964 6.44318 2.41964ZM2.91477 7.48214H6.44318C6.52756 7.48214 6.59659 7.4279 6.59659 7.36161V6.63839C6.59659 6.5721 6.52756 6.51786 6.44318 6.51786H2.91477C2.8304 6.51786 2.76136 6.5721 2.76136 6.63839V7.36161C2.76136 7.4279 2.8304 7.48214 2.91477 7.48214ZM9.20455 11.1585C9.20455 11.3183 9.28536 11.4716 9.42921 11.5846C9.57306 11.6977 9.76816 11.7612 9.97159 11.7612C10.175 11.7612 10.3701 11.6977 10.514 11.5846C10.6578 11.4716 10.7386 11.3183 10.7386 11.1585C10.7386 10.9986 10.6578 10.8453 10.514 10.7323C10.3701 10.6193 10.175 10.5558 9.97159 10.5558C9.76816 10.5558 9.57306 10.6193 9.42921 10.7323C9.28536 10.8453 9.20455 10.9986 9.20455 11.1585Z" fill="currentColor"/>\n' +
                "</svg>\n",
              requiresAuth: false,
              allowed_roles: ["admin"],
            },
          },
          {
            path: "withdrawals",
            name: "withdrawals",
            component: withdrawals,
            meta: {
              showInSideBar: true,
              verboseName: "Withdrawals",
              requiresAuth: true,
              allowed_roles: ["photographer"],
            },
          },
          {
            path: "transaction",
            name: "transaction",
            component: payments,
            props: {
              showWithdraw: true,
            },
            meta: {
              showInSideBar: true,
              verboseName: "Withdrawals",
              requiresAuth: true,
              allowed_roles: ["photographer"],
            },
          },
        ],
      },
      {
        path: "users",
        name: "users",
        component: UserList,
        meta: {
          showInSideBar: false,
          verboseName: "Users",
          requiresAuth: true,
          allowed_roles: ["photographer", "admin", "owner", "client"],
          level: 1,
          icon:
            '<svg width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">\n' +
            '<path d="M12.8864 0.25H0.613636C0.274219 0.25 0 0.465458 0 0.732143V13.2679C0 13.5345 0.274219 13.75 0.613636 13.75H12.8864C13.2258 13.75 13.5 13.5345 13.5 13.2679V0.732143C13.5 0.465458 13.2258 0.25 12.8864 0.25ZM1.38068 1.33482H12.1193V4.46875H1.38068V1.33482ZM12.1193 8.56696H1.38068V5.43304H12.1193V8.56696ZM12.1193 12.6652H1.38068V9.53125H12.1193V12.6652ZM6.44318 2.41964H2.91477C2.8304 2.41964 2.76136 2.47388 2.76136 2.54018V3.26339C2.76136 3.32969 2.8304 3.38393 2.91477 3.38393H6.44318C6.52756 3.38393 6.59659 3.32969 6.59659 3.26339V2.54018C6.59659 2.47388 6.52756 2.41964 6.44318 2.41964ZM2.91477 7.48214H6.44318C6.52756 7.48214 6.59659 7.4279 6.59659 7.36161V6.63839C6.59659 6.5721 6.52756 6.51786 6.44318 6.51786H2.91477C2.8304 6.51786 2.76136 6.5721 2.76136 6.63839V7.36161C2.76136 7.4279 2.8304 7.48214 2.91477 7.48214ZM9.20455 11.1585C9.20455 11.3183 9.28536 11.4716 9.42921 11.5846C9.57306 11.6977 9.76816 11.7612 9.97159 11.7612C10.175 11.7612 10.3701 11.6977 10.514 11.5846C10.6578 11.4716 10.7386 11.3183 10.7386 11.1585C10.7386 10.9986 10.6578 10.8453 10.514 10.7323C10.3701 10.6193 10.175 10.5558 9.97159 10.5558C9.76816 10.5558 9.57306 10.6193 9.42921 10.7323C9.28536 10.8453 9.20455 10.9986 9.20455 11.1585Z" fill="currentColor"/>\n' +
            "</svg>\n",
        },
        children: [
          {
            path: "edit-user/:userId",
            name: "edit-user",
            component: CreateUser,
            meta: {
              showInSideBar: false,
              verboseName: "Edit User",
              requiresAuth: true,
              allowed_roles: ["photographer", "admin", "owner", "client"],
              level: 1,
              icon:
                '<svg width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">\n' +
                '<path d="M12.8864 0.25H0.613636C0.274219 0.25 0 0.465458 0 0.732143V13.2679C0 13.5345 0.274219 13.75 0.613636 13.75H12.8864C13.2258 13.75 13.5 13.5345 13.5 13.2679V0.732143C13.5 0.465458 13.2258 0.25 12.8864 0.25ZM1.38068 1.33482H12.1193V4.46875H1.38068V1.33482ZM12.1193 8.56696H1.38068V5.43304H12.1193V8.56696ZM12.1193 12.6652H1.38068V9.53125H12.1193V12.6652ZM6.44318 2.41964H2.91477C2.8304 2.41964 2.76136 2.47388 2.76136 2.54018V3.26339C2.76136 3.32969 2.8304 3.38393 2.91477 3.38393H6.44318C6.52756 3.38393 6.59659 3.32969 6.59659 3.26339V2.54018C6.59659 2.47388 6.52756 2.41964 6.44318 2.41964ZM2.91477 7.48214H6.44318C6.52756 7.48214 6.59659 7.4279 6.59659 7.36161V6.63839C6.59659 6.5721 6.52756 6.51786 6.44318 6.51786H2.91477C2.8304 6.51786 2.76136 6.5721 2.76136 6.63839V7.36161C2.76136 7.4279 2.8304 7.48214 2.91477 7.48214ZM9.20455 11.1585C9.20455 11.3183 9.28536 11.4716 9.42921 11.5846C9.57306 11.6977 9.76816 11.7612 9.97159 11.7612C10.175 11.7612 10.3701 11.6977 10.514 11.5846C10.6578 11.4716 10.7386 11.3183 10.7386 11.1585C10.7386 10.9986 10.6578 10.8453 10.514 10.7323C10.3701 10.6193 10.175 10.5558 9.97159 10.5558C9.76816 10.5558 9.57306 10.6193 9.42921 10.7323C9.28536 10.8453 9.20455 10.9986 9.20455 11.1585Z" fill="currentColor"/>\n' +
                "</svg>\n",
            },
          },
          {
            path: "create-user",
            name: "create-user",
            component: CreateUser,
            meta: {
              showInSideBar: false,
              verboseName: "Edit User",
              requiresAuth: true,
              allowed_roles: ["photographer", "admin", "owner", "client"],
              level: 1,
              icon:
                '<svg width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">\n' +
                '<path d="M12.8864 0.25H0.613636C0.274219 0.25 0 0.465458 0 0.732143V13.2679C0 13.5345 0.274219 13.75 0.613636 13.75H12.8864C13.2258 13.75 13.5 13.5345 13.5 13.2679V0.732143C13.5 0.465458 13.2258 0.25 12.8864 0.25ZM1.38068 1.33482H12.1193V4.46875H1.38068V1.33482ZM12.1193 8.56696H1.38068V5.43304H12.1193V8.56696ZM12.1193 12.6652H1.38068V9.53125H12.1193V12.6652ZM6.44318 2.41964H2.91477C2.8304 2.41964 2.76136 2.47388 2.76136 2.54018V3.26339C2.76136 3.32969 2.8304 3.38393 2.91477 3.38393H6.44318C6.52756 3.38393 6.59659 3.32969 6.59659 3.26339V2.54018C6.59659 2.47388 6.52756 2.41964 6.44318 2.41964ZM2.91477 7.48214H6.44318C6.52756 7.48214 6.59659 7.4279 6.59659 7.36161V6.63839C6.59659 6.5721 6.52756 6.51786 6.44318 6.51786H2.91477C2.8304 6.51786 2.76136 6.5721 2.76136 6.63839V7.36161C2.76136 7.4279 2.8304 7.48214 2.91477 7.48214ZM9.20455 11.1585C9.20455 11.3183 9.28536 11.4716 9.42921 11.5846C9.57306 11.6977 9.76816 11.7612 9.97159 11.7612C10.175 11.7612 10.3701 11.6977 10.514 11.5846C10.6578 11.4716 10.7386 11.3183 10.7386 11.1585C10.7386 10.9986 10.6578 10.8453 10.514 10.7323C10.3701 10.6193 10.175 10.5558 9.97159 10.5558C9.76816 10.5558 9.57306 10.6193 9.42921 10.7323C9.28536 10.8453 9.20455 10.9986 9.20455 11.1585Z" fill="currentColor"/>\n' +
                "</svg>\n",
            },
          },
        ],
      },
      {
        path: "profile",
        name: "profile",
        component: UserProfile,
        meta: {
          showInSideBar: false,
          verboseName: "Profile",
          requiresAuth: true,
          allowed_roles: ["photographer", "admin", "owner", "client"],
          level: 1,
          icon:
            '<svg width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">\n' +
            '<path d="M12.8864 0.25H0.613636C0.274219 0.25 0 0.465458 0 0.732143V13.2679C0 13.5345 0.274219 13.75 0.613636 13.75H12.8864C13.2258 13.75 13.5 13.5345 13.5 13.2679V0.732143C13.5 0.465458 13.2258 0.25 12.8864 0.25ZM1.38068 1.33482H12.1193V4.46875H1.38068V1.33482ZM12.1193 8.56696H1.38068V5.43304H12.1193V8.56696ZM12.1193 12.6652H1.38068V9.53125H12.1193V12.6652ZM6.44318 2.41964H2.91477C2.8304 2.41964 2.76136 2.47388 2.76136 2.54018V3.26339C2.76136 3.32969 2.8304 3.38393 2.91477 3.38393H6.44318C6.52756 3.38393 6.59659 3.32969 6.59659 3.26339V2.54018C6.59659 2.47388 6.52756 2.41964 6.44318 2.41964ZM2.91477 7.48214H6.44318C6.52756 7.48214 6.59659 7.4279 6.59659 7.36161V6.63839C6.59659 6.5721 6.52756 6.51786 6.44318 6.51786H2.91477C2.8304 6.51786 2.76136 6.5721 2.76136 6.63839V7.36161C2.76136 7.4279 2.8304 7.48214 2.91477 7.48214ZM9.20455 11.1585C9.20455 11.3183 9.28536 11.4716 9.42921 11.5846C9.57306 11.6977 9.76816 11.7612 9.97159 11.7612C10.175 11.7612 10.3701 11.6977 10.514 11.5846C10.6578 11.4716 10.7386 11.3183 10.7386 11.1585C10.7386 10.9986 10.6578 10.8453 10.514 10.7323C10.3701 10.6193 10.175 10.5558 9.97159 10.5558C9.76816 10.5558 9.57306 10.6193 9.42921 10.7323C9.28536 10.8453 9.20455 10.9986 9.20455 11.1585Z" fill="currentColor"/>\n' +
            "</svg>\n",
        },
      },
      {
        path: "settings",
        name: "settings",
        component: SiteSettings,
        meta: {
          showInSideBar: false,
          verboseName: "Settings",
          requiresAuth: true,
          allowed_roles: ["photographer", "admin", "owner", "client"],
          level: 1,
          icon: `<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
              <path stroke-linecap="round" stroke-linejoin="round" d="M10.343 3.94c.09-.542.56-.94 1.11-.94h1.093c.55 0 1.02.398 1.11.94l.149.894c.07.424.384.764.78.93.398.164.855.142 1.205-.108l.737-.527a1.125 1.125 0 0 1 1.45.12l.773.774c.39.389.44 1.002.12 1.45l-.527.737c-.25.35-.272.806-.107 1.204.165.397.505.71.93.78l.893.15c.543.09.94.559.94 1.109v1.094c0 .55-.397 1.02-.94 1.11l-.894.149c-.424.07-.764.383-.929.78-.165.398-.143.854.107 1.204l.527.738c.32.447.269 1.06-.12 1.45l-.774.773a1.125 1.125 0 0 1-1.449.12l-.738-.527c-.35-.25-.806-.272-1.203-.107-.398.165-.71.505-.781.929l-.149.894c-.09.542-.56.94-1.11.94h-1.094c-.55 0-1.019-.398-1.11-.94l-.148-.894c-.071-.424-.384-.764-.781-.93-.398-.164-.854-.142-1.204.108l-.738.527c-.447.32-1.06.269-1.45-.12l-.773-.774a1.125 1.125 0 0 1-.12-1.45l.527-.737c.25-.35.272-.806.108-1.204-.165-.397-.506-.71-.93-.78l-.894-.15c-.542-.09-.94-.56-.94-1.109v-1.094c0-.55.398-1.02.94-1.11l.894-.149c.424-.07.765-.383.93-.78.165-.398.143-.854-.108-1.204l-.526-.738a1.125 1.125 0 0 1 .12-1.45l.773-.773a1.125 1.125 0 0 1 1.45-.12l.737.527c.35.25.807.272 1.204.107.397-.165.71-.505.78-.929l.15-.894Z" />
              <path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 1 1-6 0 3 3 0 0 1 6 0Z" />
            </svg>`,
        },
      },
    ],
  },
];

import("./authGuards");

const router = createRouter({
  history: createWebHistory("/"),
  linkExactActiveClass: "active",
  routes,
});

export default router;
